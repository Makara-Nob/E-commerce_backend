 ...
 */
appRouter.post('/api/v1/orders', async (req: IncomingMessage, res: ServerResponse) => {
    try {
 ...
 *         description: Not authorized
 */
appRouter.get('/api/v1/orders/my-orders', async (req: IncomingMessage, res: ServerResponse) => {
    try {
        const userId = await protect(req, res, appRouter);
        if (!userId) return;
 ...
 */
appRouter.get('/api/v1/orders/:id', async (req: IncomingMessage & { params?: any }, res: ServerResponse) => {
    try {
        const userId = await protect(req, res, appRouter);
        if (!userId) return;
